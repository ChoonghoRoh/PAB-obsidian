...
|---|---|
| `_attachments/` | 첨부 저장소 |
| `_INDEX.md` | vault 루트 인덱스 — TYPE 을 갖지만 수집 대상은 아니다 |
| `10_Notes/_old/` | 구버전 보관 |
| `00_MOC/TOPICS/_README.md` | 폴더 안내 |

### 규칙

- 🔴 **보관은 삭제가 아니다** — 파일은 그대로 두고 **기존 링크도 유지**한다. 옮기거나 지우지 않는다.
- `moc-build` 는 해당 문서를 **마커 구간에 넣지 않는다.**
- **이 규약은 vault 를 읽는 모든 도구에 적용된다.** 외부 수집기의 반영은 각 구현의 몫이고, **규약 자체는 이 문서가 정본**이다.
- ⚠️ **접두 문자는 양측 공통 계약이다** — 한쪽만 바꾸면 상대 도구에는 보이지 않아 **조용히 갈린다.** 변경은 **사전 통지 후 동시 적용**한다.

> **왜 이름 규약인가** — 전용 폴더(`90_Archive/` 등)로 옮기는 안도 있었으나 **파일 이동이 기존 링크를 깨뜨린다.** 실제로 `10_Notes/_old/` 안의 문서는 MOC 3곳(`DOMAINS/KNOWLEDGE_MGMT` · `TYPES/RESEARCH_NOTE` · `TOPICS/LLM_WIKI`)에 등재돼 있다. 이름 규약은 **파일을 그대로 두고 취급만 바꾼다.**

> **왜 이제 적는가** — `_` 접두는 이 vault 에서 이미 쓰이고 있던 관례였고, **규약으로 적히지 않아 도구도 수집기도 구분하지 못했다.** 2026-09-17 외부 수집 실측에서 `_old` 백업본이 현행 노트와 같은 자격으로 수집된 것이 확인되어 **관례를 규칙으로 승격한다.**
