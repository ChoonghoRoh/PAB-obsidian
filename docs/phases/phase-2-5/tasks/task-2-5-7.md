...
### 🔴🔴 `broken 206` 정체 규명 — **직전 세션의 "존재하지 않는 값" 판정을 재검토해야 한다**

2026-09-04 22건 catch-up 실행 직후 `broken`이 **185 → 206**으로 움직였다. backend-dev가 추정하지 않고 **`git worktree`로 구 vault를 재현**했다:

```
git worktree add /tmp/t7-pre HEAD    # 갱신 전 vault
갱신 전 vault → broken=206
갱신 후 vault → broken=206           # ⇒ 본 변경의 기여 0
```

**원인은 vault가 아니라 실행 경로였다.**

| 경로 | `broken` | 근거 |
|---|---:|---|
| `obsidian unresolved` (`validate.py:238`) | **185** | 어제: `rc=0` · 281줄 |
| 정규식 폴백 (`validate.py:240`) | **206** | 오늘: `rc=1` · **0줄** (3/3 재현) |
| **차이** | **21** | — |

vault 파일 수는 같은 기준으로 146으로 동일하고 어제 이후 `PAB-LLMDATA` 커밋도 없다.

> ⭐ **차이가 정확히 21이다.** 직전 세션이 *"내가 낸 오류 중 최악"* 으로 기록한 `broken 206` 건의 불일치와 **같은 값**이다. 당시 판정은 *"존재하지 않는 값을 계약 문서(PO2)에 적었고, 그 차이에 'MOC 정비로 21건 해소'라는 그럴듯한 설명까지 붙였다"* 였다.
>
> **틀린 것은 설명이었지 숫자가 아니었을 가능성이 크다.** `206`은 폴백 경로가 실제로 내는 값이다. ⇒ **"존재하지 않는 값" 판정 자체를 재검토 대상으로 등재한다.**
>
> ⚠️ 다만 이것도 아직 **추론이다.** 당시 PO2에 적힌 206이 폴백 경로에서 나온 것인지는 확인하지 않았다 — 그때의 실행 조건(obsidian 동작 여부)을 모른다. **"206은 재현 가능한 값이다"까지가 확정이고, "PO2의 206이 그것이었다"는 미확정이다.**

⇒ **`broken`은 vault가 아니라 외부 바이너리 동작 여부에 따라 21만큼 흔들린다.** `status` 판정은 `violations`·`orphans`로만 하므로 PASS는 불변이지만 **`counts.broken` 자체는 계약 필드다**(FC-14가 읽는다). `rc=1`의 원인은 규명하지 못했다 — §5 관측 항목으로 유지.
...
### ⭐ 실측된 불일치는 따로 있었다 — `TYPES/SOURCE.md`는 **얼어붙은 MOC**다

반려된 주장 대신, 그 옆에서 **진짜가 하나 나왔다.** backend-dev 재조사 + Team Lead 독립 확인.

| 측정 | 값 |
|---|---|
| `TYPES/SOURCE.md` 존재 | **예** (TYPES 디렉터리 7개 중 하나) |
| 자동 갱신 마커 | **있다** — `moc-build:auto-start` 1건 (다른 TYPE MOC 6종과 동일) |
| 현재 폴백 내용 | **`_(현재 등록된 노트 없음)_`** |
| `MOC_TYPE_NAMES`(`moc.py:16`)에 `SOURCE` | **없다** (6종만) |
| `moc-build --dry-run`에 `TYPES/SOURCE` | **0건** — 루프가 도달하지 않는다 |
| `type: "[[SOURCE]]"` 노트 | **41건** (`15_Sources/` 파일 40) |

⇒ **마커를 가진 MOC가 "등록된 노트 없음"을 표시하는데 실제로는 41건이 있다.**

> ⚠️ **구조적으로 N-5가 경고한 그 형태다** — 상수 목록이라 디스크에 있는 파일이 조용히 빠진다. T-7이 TOPICS를 **디스크 glob**으로 옮긴 이유와 **같은 종류**다. 차이는 TOPIC이 승격으로 자라는 집합이라 드러났고, `SOURCE`는 상수 목록에서 **처음부터 빠져 있어** 아무도 못 봤다는 것뿐이다.

**결함인지 의도적 제외인지는 판정하지 않는다.** R-4는 `15_Sources/*_source.md` **노트**의 불변을 규정하지 그 목록 MOC를 규정하지 않는다. 그렇다고 SOURCE를 TYPE MOC로 채우는 것이 설계 의도라는 근거도 없다. **T-7 범위 밖 — 별도 판단 대상으로 등재한다.**

> 부수 확인: `REFERENCE`가 vault 전체 **47** 대 dry-run **8**로 크게 벌어져 보이나 **허수다.** 47 중 **38이 MOC 파일 자체**(`00_MOC/TOPICS` 23 · `TYPES` 7 · `DOMAINS` 6 · `00_MOC` 2)로 `type: "[[REFERENCE]]"`를 달고 있다. 나머지 TYPE도 vault 대비 1~2건 차이뿐이다. **이상은 `SOURCE` 1건이다.**

### 반려 과정에서 드러난 것 — 원인은 `tail -35` 절단이었다

backend-dev 자기 조사 결과, 근거 명령이 `moc-build --dry-run 2>&1 | tail -35`였다. **전체 출력이 36줄이라 앞 2줄이 잘렸고, 잘린 것이 정확히 `RESEARCH_NOTE`·`CONCEPT`였다.**

> ⚠️ **더 나쁜 것은 모순을 이미 봤다는 점이다.** 같은 실행의 마지막 줄이 `총 34 MOC 인식`이었고 화면상 합계는 4+6+22=**32**였다. backend-dev는 그 차이를 알아채고 *"tail에 잘렸겠구나, TYPES는 6"* 이라고 **속으로 계산까지 했으나 최초 결론이 정정되지 않은 채 보고에 나갔다.** 측정 실패가 아니라 **관측한 모순을 끝까지 해소하지 않은 것**이다.

**감사 결과**: 이번 세션의 "없다"류 주장 8건을 근거 명령 기준으로 재점검한 결과, **절단된 목록에서 부재를 결론지은 것은 이 1건뿐**이었다. 나머지는 전부 계수 명령(`grep -c`/`wc -l`)이거나 전량 출력 대조라 절단이 개입할 수 없다.

⇒ **규율: 부재 주장은 `tail`/`head`를 거친 출력으로 하지 않는다. `ls` 또는 계수 명령으로만 한다.**

> ⚠️ **Team Lead도 같은 반려 과정에서 측정 오류를 냈다.** `type: *\[\[SOURCE\]\]`로 grep해 **0건**을 얻고 *"backend-dev의 40건이 재현되지 않는다"* 고 쓸 뻔했다. 실제 표기는 `type: "[[SOURCE]]"` 로 **따옴표가 있어** 패턴이 빗나간 것이다. frontmatter 원문을 열어보고서야 알았다. — **패턴이 0을 반환한 것과 대상이 0인 것은 다르다.**
