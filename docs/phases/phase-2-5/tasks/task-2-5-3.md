...
### 서버 cron — **2줄** ✅ **등록 완료 (2026-09-03 10:05)**
...
#### 등록 실행 기록 (2026-09-03 10:05, backend-dev / Team Lead 검증)

Observer OB2-C §8.4로 정식 수용 → 사용자 승인 → `deploy_monitoring.sh --volbackup-only` 실행.

| 검증 항목 | 실측 |
|---|---|
| crontab | 8줄 → **10줄**. 기존 8줄 **전건 무변경** |
| 시각 분리 | Observer `17 3` 바로 뒤 우리 `17 4` — **1시간 분리 확인** |
| 프라이밍 | 백업 1회 + 복원 리허설 1회. `LAST_OK_TS`/`LAST_VERIFY_TS` → 09-03 10:05 |
| 리허설 실측 | `LAST_VERIFY_DOCS=3064` · `LAST_VERIFY_LOCAL=13` · **`LAST_VERIFY_DDOC=194`**(VDU 본문 길이) |
| `carry()` 상호 보존 | `LAST_SIZE=1492199` · `TOTAL_SIZE=5961078` 유지 — 리허설이 백업 메타를 지우지 않음 |
| ⭐ **T-1 판정** | `check_couchdb_volbackup` → **OK** (경과 0h, 세대 4/7, 리허설 0일 전) |
| 가동본 무접촉 | `pab-couchdb Up 10 days (healthy)` — uptime 무단절 |
| 임시 자원 정리 | `restoretest` 컨테이너 **0** / 볼륨 **0** |
| 롤백 | `state/crontab.bak-20260903-100515` (600, 819B) — `crontab <파일>` 한 줄로 복구 |

> ⭐ **프라이밍을 기본 ON으로 둔 것이 핵심이었다.** `pab_sync_healthcheck.sh:351`이 `PAB-COUCHDB-VOLBACKUP` 마커를 grep해 감시를 켜므로 **등록이 곧 감시 스위치 ON**이다. 등록 시점의 `LAST_OK_TS` 경과는 **196.7h**(임계 36h)였고, 프라이밍 없이 등록했다면 **즉시 FAIL → 15분 뒤 Telegram 오탐**이었다.
>
> backend-dev가 한 발 더 나갔다 — `LAST_VERIFY_TS`는 09-05에 만료되는데 첫 리허설 cron은 **09-06(일) 04:42**이라 **24h 오탐 구간이 추가로** 생긴다. 그래서 백업만이 아니라 **리허설도** 프라이밍해야 했다. Team Lead는 "10일 임계까지 1.81일 여유"에서 멈췄고 다음 발화 시각을 보지 않았다.

> ⚠️ **검증은 "마커가 grep되는가"가 아니라 "판정이 OK인가"까지 봤다.** 마커 존재는 감시가 켜졌다는 증거일 뿐 오탐이 없다는 증거가 아니다 — 이 Phase에서 여덟 번 반복된 오류 형태를 피하기 위한 절차다.

#### 미측정 (추정으로 적지 않는다)

- **세대 회전** — 현재 4세대 < `KEEP=7`이라 회전이 한 번도 발동한 적이 없다. 백업 파일 소유자가 `root:root`이고 unlink 권한은 디렉토리(`oceanui`, 700, sticky 없음)에서 나오므로 성공이 예상되나 **논증했을 뿐 실측하지 않았다.** 7세대 도달(09-06 이후)에 실측 필요
