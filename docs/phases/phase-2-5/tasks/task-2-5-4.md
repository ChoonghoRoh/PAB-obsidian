...
> ⭐ **정정 (2026-09-04, Team Lead 재실측).** 위 문단의 *"VDU 에 닿기 전에 성공했을 수도 있다"* 는 **근거가 약하다.**
> §A 가 이미 기록한 VDU 원문을 다시 읽었다 — `userCtx.name` **하나만 보고 문서 내용은 전혀 보지 않는다.**
> 따라서 `pabprove` 에게는 *"닿기 전 성공"* 과 *"닿아서 통과"* 의 **결과가 같다.**
>
> ⇒ **그럼에도 ①을 유지한다. 다만 이유가 바뀐다.**
> VDU 원문만으로도 *"이름이 다르니 통과한다"* 는 **논증**은 선다. 그러나 그 논증은
> *"CouchDB 는 유효 문서 쓰기 때 반드시 VDU 를 호출한다"* 와 *"`_` 접두는 VDU 이전에 400 으로 걸린다"*
> **두 가정에 기대고 있고, 둘 다 문서상 계약일 뿐 이 인스턴스에서 우리가 재보지 않았다.**
> **①의 역할은 "페이로드의 도달성 증명"이 아니라 "그 두 가정을 한 번에 실측으로 바꾸는 것"이다.**
>
> ⚠️ 실행 순서 제약은 그대로다 — **①이 400 이면 페이로드를 고쳐 ①부터 다시 한다.**
> 그 상태로 ②를 실행하면 어떤 결과가 나와도 해석할 수 없다.
...
### H. ⭐ `_security` 를 건드리지 않는 편입 경로가 있을 수 있다 (2026-09-04, 미검증)

§B ⑵ 는 *"`pabprove` 는 `_security.members` 에 없으면 읽기도 안 된다"* 로 정리했다. **그 자체는 맞다. 다만 members 는 `names` 만이 아니다.**

§A 실측값을 다시 보면:

```
members.names = ["pabbridge"]
members.roles = ["_admin", "bridge_ro"]      ← 여기
```

**`bridge_ro` 롤을 가진 계정은 이름이 목록에 없어도 member 로 인정된다.** 그렇다면 `pabprove` 를 만들 때 `_users` 문서에 `roles: ["bridge_ro"]` 를 주는 것만으로 **`_security` 를 전혀 수정하지 않고** 접근이 열릴 수 있다.

**왜 중요한가** — `_security` 변경은 이 Task 에서 가장 위험한 지점이었다:

| `_security` 를 고칠 때 | 롤 부여로 우회할 때 |
|---|---|
| `setup-readonly-account.sh` 축출 결함의 사정권 (1단계에서 닫았으나 재발 여지) | 사정권 밖 |
| `deploy.sh` 자동 호출이 언제든 덮어쓸 수 있음 | 영향 없음 |
| 롤백 = 원문 재PUT | 롤백 = `_users` 문서 rev 복원 (더 안전) |

⚠️ **다만 부채가 하나 붙는다** — 롤 이름이 `bridge_ro`(bridge **read-only**)인데 `pabprove` 는 **쓰기가 필요한** 계정이다. VDU 는 이름만 보므로 `bridge_ro` 롤이어도 `pabprove` 이름이면 쓰기가 통과한다. **롤 이름과 실제 권한이 어긋나 다음 사람을 오도한다** — 1단계에서 닫은 결함(주석과 코드가 반대)과 **같은 형태의 부채**다.

⇒ **선택지 3개. 판단은 실행 전에 한다.**

| 안 | 내용 | 비용 |
|---|---|---|
| ⒜ `members.names` 에 `pabprove` 추가 | 종전 계획 | `_security` 변경 1회 (병합은 이미 안전) |
| ⒝ `pabprove` 에 `bridge_ro` 롤 부여 | `_security` 무변경 | **롤 이름이 오도** |
| ⒞ 전용 롤 신설 후 `members.roles` 에 추가 | 의미 정합 | `_security` 변경 1회 + 롤 1개 증가 |

> 🔴 **전부 미검증이다.** *"`members.roles` 에 `bridge_ro` 가 있다"* 는 **실측**이고,
> *"그 롤을 가지면 목록 밖 계정도 접근된다"* 는 **CouchDB 문서상 동작을 근거로 한 추론이다 — 이 인스턴스에서 재보지 않았다.**
> `pabprove` 발급 시 ⒜/⒝ 중 무엇을 택하든, **접근 가능 여부는 실제 GET 으로 확인하고 나서 다음 단계로 간다.**
