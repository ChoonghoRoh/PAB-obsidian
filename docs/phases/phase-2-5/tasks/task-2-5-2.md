...
## ⚠️ G9 배치 결함과 수정 (2026-08-26) — 가드가 자기 시나리오에서만 침묵했다

**초판(`beac193`)은 G9를 커밋 직전(G8 다음)에 두었다.** 구현 자체는 요구사항을 다 지켰고 시험 4종도 통과했다. 그런데 **정작 막으려던 상황에서 발화하지 않았다.**

### 결함 경로

```
vault 노트에 skip-worktree 비트  →  사용자가 그 노트를 편집
  →  `git add -A` 가 건너뜀  →  다른 변경이 없으면 CHANGED_N = 0
  →  ":변경 없음 — 무소음 종료(no-op)"  →  exit 0  →  ⚠️ G9 미도달
```

**누락이 일어난 바로 그 순간에 침묵**하고, 다른 파일이 우연히 함께 바뀐 때에만 경고가 떴다 — **가장 필요할 때 조용하고 덜 필요할 때 우는 가드**였다.

### 왜 시험이 통과했는가

backend-dev 시험 4종(비트 0건 / 1건 / 6건+상한 / 커밋 성공)이 **전부 "커밋할 변경이 있는" 경로**였다. `CHANGED_N > 0`이라 G9에 도달했다. **가장 중요한 분기를 우연히 비켜갔다.**

Team Lead가 **작업트리를 비운 상태에서 실제로 비트를 걸어** 재현하며 드러났다. 코드 리뷰만으로는 통과했을 결함이다.

### 수정 (`e7da951`)

`detect_skip()` 함수로 분리해 **락 획득 직후(`:214`)** 에서 호출 — `g add -A`(`:271`)·no-op 판정(`:288`)보다 앞이라 **no-op·abort·커밋 모든 경로에서 발화**한다. 비트 감지는 스테이징 결과와 무관하므로(`git ls-files -v`만 보면 된다) 앞으로 옮겨도 정확도가 같다.

**⚠️ 되돌리지 말 것.** 커밋 직전이 더 자연스러워 보이지만, 그 위치에서는 위 결함이 그대로 재발한다. 코드 `:126-131`에 같은 경고를 남겼다.

### 검증 (Team Lead 독립 실측, 작업트리 clean 상태)
...
| `--status` | ✅ 발화 (검증자 편의) |
| 무소음 원칙 | ✅ 로그·커밋 메시지 전용, **Telegram 미발송** — "변경 없음"은 정상 상태이고 비트 감지는 경고이지 이상이 아니다 |
| 비트 원복·잔재 | ✅ 0건, `Makefile` 내용 무변조 |

HR-5 390줄, PR-3 병합 명령 0건 유지.

> **"가드를 넣었다"는 "가드가 동작한다"가 아니다.** T-3의 *"백업했다 ≠ 복구된다"*, T-5의 *"`git status`가 조용하다 ≠ 문제가 없다"*, `git check-ignore` 0건 ≠ 없다 와 같은 계열이다. **시험이 초록 불을 켰다는 사실이 그 시험이 옳은 것을 봤다는 사실은 아니다.**
