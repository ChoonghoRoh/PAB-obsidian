...
| **OB2-E** | ⭐ **probe ① 대행 요청**(`E-1`) + **T-4 회전 작업 창 합의 요청**(`E-2`) — 둘 다 T-4 차단 해제 경로 · **T-3 확정 PASS**(회전 2일 연속·복원 리허설 무인) · `counts.broken` **원인 규명**(Obsidian 앱 기동 여부, `status` 불변으로 FC-14 영향 없음) · 쓰기 경로 고지(`obsidian create`) · 홉 구조 **예고**(실물 아님, `#31` **선등재 금지**) · ⚠️ **`OB2-D` 배치 누락 보고** · 회신 요청 `E-1`~`E-5` | **PAB-obsidian** | 회신본 | [`260908-OB2E-probe대행요청+T4회전작업창+T3확정PASS.md`](260908-OB2E-probe대행요청+T4회전작업창+T3확정PASS.md) |
| **OB2-F** | 🔴 **`OB2-E` §4.4 철회**(수집기는 재귀 스캔 — 고장 형태가 침묵이 아니라 **오염**, 위험 방향이 반대였다) · 🔴 **`E-1` probe ① 전제 변경**(VDU 실측 — `userCtx.name === 'pabbridge'` **이름만 검사**, `_security`는 롤로도 멤버를 받으므로 `bridge_ro` 롤의 **다른 이름 계정은 쓸 수 있다**. probe는 그 위험을 재지 않는다) · **회원 vault 실물 발생**(단 LiveSync 밖 — `#31` 선등재 금지 유효) · 채번 **접두 분리 제안** · 회신 요청 `F-1`~`F-3` | **PAB-obsidian** | 회신본 | [`260911-OB2F-OB2E정정+probe전제변경+회원vault실물.md`](260911-OB2F-OB2E정정+probe전제변경+회원vault실물.md) |
