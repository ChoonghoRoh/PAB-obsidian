...
> **SSOT**: v8.2-renewal-6th | **용도**: 모든 SUB-SSOT에서 공통으로 참조하는 포맷·규칙 정의
...
**적용 시점**: AutoCycle 전 사이클 (1 사이클 = Step 1~14 기준 — "15단계" 표기는 사용자 주도 시 Step 0 Pre-draft 포함. Step 0은 본 토큰 상한 산정에서 제외)
...
| **Step 0 Pre-draft (사용자 주도)** | 30,000 tokens (권장) | 사이클 500K와 **독립 예산** — 초과 시 토픽 논의 범위 축소 |
...
> ⚠ **집행 방식 — 현재 수동 운영**: 토큰 소비를 자동 집계하는 스크립트/훅은 **번들에 탑재되어 있지 않다**. §8.2의 80% WARNING·100% HALT는 Team Lead가 수동 추적으로 집행하는 규칙이며, 자동으로 발동되지 않는다. 따라서 Team Lead는 **매 Step 전환 시점에 소비량 기록을 생략하지 말 것** — 기록 누락 시 상한 방어가 무력화된다.

- **Team Lead**: 각 Phase/Step 전환 시 토큰 소비량 기록 — 기록 위치는 **`phase-achievement-report.md §3.5`(ITERATION-BUDGET 토큰 소비 기록)** 와 **`status.md` 의 `token_budget: { limit, consumed, last_recorded_at }`** 두 곳이다. 둘이 어긋나면 `status.md` 를 정본으로 삼는다
- **verifier**: G2 검증 시 토큰 예산 초과 여부 확인
- **예외**: 사용자가 사전에 "예산 무제한"을 선언한 경우 본 규칙 적용 제외 (선언 기록 필수)

> 🔴 **미기록은 N/A가 아니라 FAIL(HIGH)이다.** `status.md` 의 `token_budget.consumed` 가 `0` 이거나 `last_recorded_at` 이 `null` 인 채로 G2에 진입하면 verifier는 ITERATION-BUDGET 항목을 **FAIL** 로 판정한다. **"관측 수단이 없어 판정 불가"는 더 이상 유효한 사유가 아니다.**
>
> **왜 이 조항이 필요한가**: Phase 9-1 G2에서 이 항목이 N/A로 나왔는데, 원인은 관측 수단의 부재가 아니라 **규정이 지목한 기록 위치가 빈 자리**였다는 데 있었다 — §8.3이 `phase-achievement-report.md §3` 을 지목했으나 그 §3은 「미완성 항목 분석」이고 템플릿 75행에 토큰 필드가 0건이었다(Phase 9-2 실측). 자리를 만들어도 **미기록을 N/A로 받아주면 회피 경로가 그대로 남아** 다음 Phase에서 같은 N/A가 반복된다.

**비목표 (과장 금지)**: 자동 집계 스크립트·훅은 본 절의 **범위 밖**이다. 본 절이 확정하는 것은 **계기**(어디에 무엇을 기록하는가)와 **판정 규칙**(미기록을 어떻게 다루는가)이며 집계 자동화가 아니다. 훅 기반 자동 집계는 `SubagentStart`/`SubagentStop` 이 agent team teammate 에 미발화하는 실측 제약(pre-analysis §10.1 — 실험군 0건 / 대조군 `PostToolUse` 2건)에 부딪히며, 이는 SPAWN_GATE가 만난 것과 **같은 벽**이다. 위 «집행 방식 — 현재 수동 운영» 고지는 그대로 유지한다.
