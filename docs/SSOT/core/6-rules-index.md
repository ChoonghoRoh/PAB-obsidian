...
> **버전**: 1.5 | **최종 갱신**: 2026-05-05 (Phase 7-1 — WT 카테고리 WT-6/WT-7 추가, 총수 100 → 102) · **ver6-1 PoC 갱신**: 2026-05-22 (LIFECYCLE-5 추가, 총수 102 → 103) · **ver6-2 PoC 갱신**: 2026-08-08 (Phase 8-3 — LIFECYCLE-6 추가, 총수 103 → 104)
> **SSOT**: v8.2-renewal-6th (AutoCycle v1.1) | **생성**: Phase 24-1 Task 24-1-1 / Phase 7-1 갱신

## 개요

5th/6th SSOT에 정의된 모든 규칙의 통합 인덱스. 3중 색인(카테고리/파일/심각도)을 제공한다.

- **총 규칙 수**: 104개 상위 규칙 (하위 규칙 포함 138개, ver6-1 PoC: LIFECYCLE-5 +1 · ver6-2 PoC: LIFECYCLE-6 +1)
- **카테고리**: 20개
- **원본 파일**: 7개
- **심각도 분포**: CRITICAL 44 / HIGH 41 / MEDIUM 18 / LOW 1 (Phase 7-1 — WT-6/WT-7 HIGH 2건 추가; ver6-1 PoC — LIFECYCLE-5 CRITICAL 1건 추가; ver6-2 PoC — LIFECYCLE-6 CRITICAL 1건 추가; 합계 104)
...
| **ITERATION-BUDGET** | 사이클 자원 상한 | 1 사이클 토큰 상한 500K. 80% WARNING, 100% HALT + 에스컬레이션. 사용자 "예산 무제한" 선언 시 예외. **계기(Phase 9-2)**: `status.md` `token_budget` + `phase-achievement-report.md §3.5`. **미기록은 N/A가 아니라 G2 FAIL(HIGH)** | core/7-shared-definitions §8·§8.3, 3-workflow Step 8 | **CRITICAL** |
...
### 1.15 LIFECYCLE — 에이전트 라이프사이클 관리 — 6개
...
| LIFECYCLE-5 | 좀비 감지 + Respawn | spawn 30초 1차 + 3분 정기 check. 좀비 확정 시 shutdown_request + suffix(`_r1`~`_r5`) respawn, 상한 5회 도달 시 BLOCKED | 3-workflow §AGENT-LIFECYCLE, SUB-SSOT TEAM-LEAD `2-lifecycle-procedure.md` §LIFECYCLE-5 RESPAWN | CRITICAL |
| LIFECYCLE-6 | 체크 스케줄러 (Phase 8-3) | LIFECYCLE-1·5의 실행체. 팀원 ≥1 Phase는 BUILDING 진입 전 워처 arm 필수(미arm 시 진입 차단). 3분 폴링 + spawn+30초 1차 체크, 상태 변화 시에만 알림. **하위 LIFECYCLE-6.1 SPAWN_GATE**(Phase 9-2): 1차 체크를 30초→**60~90초**로 확장 + 판정 비대칭(무응답 = `SPAWN_UNCONFIRMED`, 자동 respawn 금지) | 3-workflow §AGENT-LIFECYCLE, SUB-SSOT TEAM-LEAD `2-lifecycle-procedure.md` §LIFECYCLE-6 SCHEDULER · §SPAWN_GATE | CRITICAL |
...
### 1.21 WT — Worktree (병렬 격리) — 7개

> **원본**: [3-workflow.md §6.6](../3-workflow.md#66-worktree-규칙-wt-1--wt-5) / 상세: `/worktree` skill
> **배경**: 현행 SSOT §7.3 병렬 처리 정책의 "수정 파일 집합 교집합 ∅" 조건만으로는 빌드 산출물·git 상태 경합을 막지 못함. worktree 격리로 3요소(checkout·stash·빌드 캐시) 동시 해결.
> **Phase 7-1 추가**: WT-6 (휴리스틱 룰 D1~D4) + WT-7 (5단 게이트 체인 G-A~G-E) — worktree 발동 인지 메커니즘 통합 (정본 [01-design-decisions.md rev.2 APPROVED](../../../poc/worktree-gate-design/01-design-decisions.md)).
...
| **WT-6** | **휴리스틱 룰 (Phase 7-1)** | 신호 셋 S1+S5+S6 기반 worktree 발동 3분 판정 (필요/권장/불필요). 우선순위 S6 > S5 > S1. **자동 발동 금지** (사용자 통제권 보장). 정본 §1 D1~D4 | [3-workflow §6.6.1](../3-workflow.md#661-wt-6--worktree-발동-휴리스틱-룰-phase-7-1-신규) | **HIGH** |
| **WT-7** | **5단 게이트 체인 (Phase 7-1)** | G-A(Plan 휴리스틱 안내) → G-B(phase-init YAML 자동 삽입) → G-C(BUILDING 차단) → G-D(verifier G2 적발) → G-E(VERIFYING 후 차기 Phase 권고). 단일 게이트 누락 안전망 보장. G-C 차단 블록은 [3-workflow §3 BUILDING](../3-workflow.md#3-워크플로우-실행) 에 명시 | [3-workflow §6.6.2](../3-workflow.md#662-wt-7--worktree-발동-5단-게이트-체인-phase-7-1-신규) + [§3 BUILDING G-C](../3-workflow.md#3-워크플로우-실행) | **HIGH** |

**심각도 분포**: CRITICAL 2 (WT-1, WT-3) / HIGH 4 (WT-2, WT-4, **WT-6, WT-7**) / MEDIUM 1 (WT-5). (총 7개, Phase 7-1 갱신)
...
| `5th/3-workflow.md` | CHAIN-1~10, REFACTOR-1~3, E0~E4, G0~G4, ASSIGN-1~5, LIFECYCLE-1~6, REPORT-1~5, NOTIFY-1~3, ANALYSIS-1~3, **WT-1~5 (정본 §6.6)** | 50 |
...
| **WT-1, WT-3** | **Worktree** | **병렬 N ≥ 2 worktree 필수 / CWD 일관성 (위반 시 즉시 중단)** |

### HIGH (27개 표시 / 메타 합계 41개) — Phase 7-1 +2 (WT-6, WT-7)

> **참고**: 본 §3 표는 카테고리 대표 행만 표시. 메타 합계(L13 HIGH 41)와 차이는 §1 다른 카테고리(CHAIN-11~13, LIFECYCLE-3~4, REPORT-1/2/4/5, NOTIFY-2~3, PROMPT-QUALITY 등)에 산재된 HIGH 룰이 §3에 나열되지 않은 사전 표시 결함. 본 Phase 7-1 범위 외.
...
| REFERENCE-1 | 문서 우선순위 |
| ANALYSIS-2, 3 | 사전 분석 |
| **WT-2, WT-4, WT-6, WT-7** | **Worktree** (경로 규약 · 수명 주기 · **휴리스틱 룰** · **5단 게이트 체인**) |
...
- **규정 상세**: ../refactoring/refactoring-rules.md
...
## §6 CRITICAL Quick Reference (41개)
...
| LIFECYCLE-5 | 좀비 감지(30초/3분 check) → respawn(상한 5회) → 초과 시 BLOCKED |
| LIFECYCLE-6 | 팀원 ≥1 Phase는 BUILDING 진입 전 워처 arm 필수, 미arm 시 진입 차단 |
