...
- **목적**: `docs/SSOT/core/` 를 별도 리포지토리(`ssot-core`)로 분리
- **이유**: Core 프레임워크의 이식성 확보, 다른 프로젝트에서 git subtree로 Core만 가져와 사용
- **전제 조건**: `core/` 물리 집결 완료 (현재 미수행 — 참조 README 단계, [core/README.md](../core/README.md) 참조)
...
cp -r {프로젝트루트}/docs/SSOT/core/* ssot-core/
cd ssot-core
git add -A
...
cd {프로젝트루트}
# 기존 core/ 백업
mv docs/SSOT/core docs/SSOT/core.bak
# subtree 추가
...
```bash
git subtree push --prefix=docs/SSOT/core https://github.com/{user}/ssot-core.git main
```

### ssot-core 변경 -> 프로젝트 반영 (pull)

```bash
git subtree pull --prefix=docs/SSOT/core https://github.com/{user}/ssot-core.git main --squash
```
...
# Core subtree push
core-push:
    git subtree push --prefix=docs/SSOT/core {REMOTE} main

# Core subtree pull
core-pull:
    git subtree pull --prefix=docs/SSOT/core {REMOTE} main --squash
```

## 8. 실행 타임라인

- **(향후 선택 작업)**: ssot-core 리포지토리 생성 + 초기 분리 실행
- **분리 전 core/ 파일 이동 완료 필요**: 현재 일부 Core 파일은 아직 `docs/SSOT/` 루트에 위치
