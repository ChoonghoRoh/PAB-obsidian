...
| 항목 | 내용 |
|------|------|
| **프로젝트명** | Personal AI Brain v3 (예시 — 실제 프로젝트 정보는 루트 [PROJECT.md](../../PROJECT.md) §1) |
| **목적** | 로컬 설치형 개인 AI 브레인 — 문서 벡터화, 의미 검색, AI 응답, 지식 구조화, Reasoning |
| **배포 형태** | Docker Compose (On-Premise, 폐쇄망 동작 필수) |
| **현재 Phase** | `docs/phases/phase-X-Y/phase-X-Y-status.md` 참조 (없으면 NO ACTIVE PHASE) |
...
|------|---------|----------|:---------------:|:-------:|:--------:|:----------:|
| **Team Lead** | `LEADER.md` | — (메인 세션) | — | opus 계열 최신 | ❌ | — (조율·판정) |
| **Research Lead** | — | `research-lead` | `Explore` | opus 계열 최신 | ❌ | — (조사 총괄) |
| **Research Architect** | — | `research-architect` | `Explore` | opus 계열 최신 | ❌ | — (아키텍처 탐색) |
| **Research Analyst** | — | `research-analyst` | `Explore` | opus 계열 최신 | ❌ | — (코드 분석) |
| **Planner** | — (고유) | `planner` | `Plan` 또는 `Explore` | opus 계열 최신 | ❌ | — (계획 수립) |
| **Backend Developer** | `BACKEND.md` | `backend-dev` | `general-purpose` | sonnet 계열 최신 | ✅ | `[BE]` `[DB]` `[FS]`(BE) |
| **Frontend Developer** | `FRONTEND.md` | `frontend-dev` | `general-purpose` | sonnet 계열 최신 | ✅ | `[FE]` `[FS]`(FE) |
| **Verifier** | `QA.md` | `verifier` | `Explore` | opus 계열 최신 | ❌ | — (코드 리뷰) |
| **Tester** | `QA.md` | `tester` | `Bash` | sonnet 계열 최신 | ❌ | — (테스트 실행) |

*backend-dev·frontend-dev·tester: sonnet 계열 최신 기본. Team Leader 판단·승인 시 일시 opus 계열 승격 가능 (§7.2).*
...
|------|------|
| **팀원 이름** | `planner` |
| **실행 방법** | `Task tool` → `team_name`, `name: "planner"`, `subagent_type: "Plan"`, `model: "opus 계열 최신"` |
...
➜ [ROLES/planner.md](ROLES/planner.md) · [SUB-SSOT/PLANNER/1-planning-procedure.md](SUB-SSOT/PLANNER/1-planning-procedure.md)
...
|------|----------|:---------------:|:-------:|----------|
| **Research Lead** | `research-lead` | `Explore` | opus 계열 최신 | 조사 총괄, Research Team 지휘, G0 산출물 품질 관리 |
| **Research Architect** | `research-architect` | `Explore` | opus 계열 최신 | 아키텍처 대안 탐색, 기술 스택 비교, PoC 설계 |
| **Research Analyst** | `research-analyst` | `Explore` | opus 계열 최신 | 기존 코드베이스 분석, 의존성·영향 범위 조사, 데이터 수집 |
...
| **팀원 이름** | `backend-dev` |
| **실행 방법** | `Task tool` → `team_name`, `name: "backend-dev"`, `subagent_type: "general-purpose"`, `model: "sonnet 계열 최신"` |
...
| **팀원 이름** | `frontend-dev` |
| **실행 방법** | `Task tool` → `team_name`, `name: "frontend-dev"`, `subagent_type: "general-purpose"`, `model: "sonnet 계열 최신"` |
...
| **팀원 이름** | `verifier` |
| **실행 방법** | `Task tool` → `team_name`, `name: "verifier"`, `subagent_type: "Explore"`, `model: "opus 계열 최신"` |
...
| **보고서 저장 절차** | verifier는 read-only이므로 보고서 본문을 SendMessage 인라인으로 Team Lead에 전달하고, Team Lead가 backend-dev에 대리 저장 task 위임. 상세: [ROLES/verifier.md §5](ROLES/verifier.md#5-보고서-저장-정책-대리-저장-패턴--phase-3-2부터-정식-적용) |
...
| **Charter** | [QA.md](PERSONA/QA.md) (4th PERSONA) |
| **팀원 이름** | `tester` |
| **실행 방법** | `Task tool` → `team_name`, `name: "tester"`, `subagent_type: "Bash"`, `model: "sonnet 계열 최신"` |
...
[1.5] (5th) Research Team 스폰 (5th_mode.research = true 시) ← 선택적
  │   Task(research-lead, Explore/opus 계열 최신), Task(research-architect, Explore/opus 계열 최신),
  │   Task(research-analyst, Explore/opus 계열 최신)
  │   → RESEARCH → RESEARCH_REVIEW(G0) 통과 후 PLANNING
  │
  ▼
[2] Task tool(team_name, name, subagent_type, model) × N  ← 팀원 스폰
  │   예: Task(team_name: "phase-15-1", name: "planner", subagent_type: "Plan", model: "opus 계열 최신")
  │       Task(team_name: "phase-15-1", name: "backend-dev", subagent_type: "general-purpose", model: "sonnet 계열 최신")
  │
  ▼
...
[1.5b] Research Team 스폰 (3인)
  │   Task(team_name, name: "research-lead", subagent_type: "Explore", model: "opus 계열 최신")
  │   Task(team_name, name: "research-architect", subagent_type: "Explore", model: "opus 계열 최신")
  │   Task(team_name, name: "research-analyst", subagent_type: "Explore", model: "opus 계열 최신")
  │
  ▼
...
| **모델 선택** | Team Lead·Planner·Verifier·Research(L/A/A): `opus 계열 최신`. Backend Dev·Frontend Dev·Tester: `sonnet 계열 최신` (기본). **예외**: Team Leader 판단·승인 시 일시 opus 계열 승격 가능 (§7.2) |
| **지연 스폰** | verifier, tester는 VERIFYING/TESTING 단계 진입 시 스폰 (비용 절감) |
...
### 7.2 모델 사용 정책 (Phase 3-2 명문화, 2026-05-01)

| 분류 | 모델 | 적용 역할 |
|------|------|----------|
| **opus 계열 최신** | 판단·검토·아키텍처 | Team Lead, Planner, Verifier, Research(Lead/Architect/Analyst) |
| **sonnet 계열 최신** (기본) | 구현·검증 | Backend Dev, Frontend Dev, Tester |
| **예외 (승격)** | sonnet → opus 일시 | Team Leader에게 요청 → Team Leader 판단·승인 |
...
**현 시점 모델 ID** (참고용, 가변): opus 계열 → `claude-opus-4-7`, sonnet 계열 → `claude-sonnet-4-6`
...
| **Plan·Leader 작업 지시 별도** | 병렬 처리 Phase에서는 **planner**가 계획 시, **Task별 수정 파일 경로·담당 팀원**을 명시하고, 병렬 가능 쌍에 대해 **작업 지시를 트랙별로 구분**하여 출력. **Team Lead**는 BUILDING 진입 시 병렬 팀원에게 **SendMessage를 트랙별로 별도 전달** (Task A → backend-dev-1, Task B → backend-dev-2 등). [SUB-SSOT/PLANNER/1-planning-procedure.md](SUB-SSOT/PLANNER/1-planning-procedure.md), [PERSONA/LEADER.md](PERSONA/LEADER.md) §4 참조. |
...
| **Worktree 필수 (WT-1)** | 병렬 BUILDING 트랙 ≥ 2 시 `git worktree` 로 작업 디렉토리 격리 필수. **수정 파일 집합 교집합 ∅ 조건만으로는 빌드 산출물(`node_modules`·`.venv`·`__pycache__`)·`git checkout`·`git stash` 경합을 막지 못함**. 병렬 트랙 수 N 판정 후 N ≥ 2 시 `BRANCH_CREATION → WORKTREE_SETUP` 전이 강제. 상세: `/worktree` skill, [3-workflow.md §6.6](3-workflow.md#66-worktree-규칙-wt-1--wt-5). |
...
| **G3 pytest 실행** | 결과를 받으려면 **동기 실행** 권장. 백그라운드가 불가피하면 stdout을 `> /tmp/agent-messages/phase-X-Y-pytest.log` 등 **이 경로**로 리다이렉트한 뒤, 완료 시 **그 파일만** 읽음. ➜ [SUB-SSOT/TESTER/1-testing-procedure.md](SUB-SSOT/TESTER/1-testing-procedure.md) |
| **테스트 요청·결과 기록(1주기)** | 요청서(목록)+결과서를 **docs/pytest-report/** 에 `YYMMDD-HHMM-phase-X-Y-테스트명.md` 로 저장. `run_tester_with_report.py` 참조. ➜ [SUB-SSOT/TESTER/1-testing-procedure.md](SUB-SSOT/TESTER/1-testing-procedure.md) |
