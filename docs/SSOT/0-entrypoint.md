# SSOT 진입점 (v8.2-renewal-6th) — 단독 사용
...
- **본 docs/SSOT 세트만으로 단독 사용 가능** (다른 SSOT 폴더 참조 불필요).
...
| **프로젝트** | 루트 [PROJECT.md](../../PROJECT.md) §1 참조 (프로젝트별 설정 단일 소스) |
| **현재 Phase** | `docs/phases/phase-X-Y/phase-X-Y-status.md` 참조 — 없으면 **NO ACTIVE PHASE** (ENTRY-1) |
...
| **Backend Dev** | 코어 2(BE) + 3 + [SUB-SSOT/DEV/](SUB-SSOT/DEV/) (CODER 전용 — v1.1) + [ROLES/backend-dev.md](ROLES/backend-dev.md) (PERSONA: [BACKEND.md](PERSONA/BACKEND.md)) |
| **Frontend Dev** | 코어 2(FE) + 3 + [SUB-SSOT/DEV/](SUB-SSOT/DEV/) (CODER 전용 — v1.1) + [ROLES/frontend-dev.md](ROLES/frontend-dev.md) (PERSONA: [FRONTEND.md](PERSONA/FRONTEND.md)) |
...
**계획 시작 시**: [SUB-SSOT/PLANNER/1-planning-procedure.md](SUB-SSOT/PLANNER/1-planning-procedure.md) 참조
...
**Task 시작 시**: [SUB-SSOT/DEV/1-fn-procedure.md](SUB-SSOT/DEV/1-fn-procedure.md) 참조
...
**검증 시작 시**: [SUB-SSOT/VERIFIER/1-verification-procedure.md](SUB-SSOT/VERIFIER/1-verification-procedure.md) 참조
...
**테스트 시작 시**: [SUB-SSOT/TESTER/1-testing-procedure.md](SUB-SSOT/TESTER/1-testing-procedure.md) 참조

**SUB-SSOT**: [SUB-SSOT/TESTER/](SUB-SSOT/TESTER/) — 모듈형 로딩 시 `core/7-shared + TESTER/0~1` 로딩

**핵심 원칙**: pytest 실행, E2E 실행, 커버리지 80%, Team Lead 경유 보고, **Ollama/AI 테스트(`tests/test_ai_api.py`, `/api/ask`) 병렬(동시) 실행 금지(단일 pytest만 실행)**. G3 결과 확인 시 **동기 실행** 권장; 다른 경로(도구 task output 등) sleep 후 반복 조회 금지. ➜ [SUB-SSOT/TESTER/1-testing-procedure.md](SUB-SSOT/TESTER/1-testing-procedure.md)

**결과 기록**: `/tmp/agent-messages/`(롤 넘기기) + **docs/pytest-report/** (1주기 요청서·결과서, `YYMMDD-HHMM-phase-X-Y-테스트명.md`). ➜ [SUB-SSOT/TESTER/1-testing-procedure.md](SUB-SSOT/TESTER/1-testing-procedure.md)
...
Team Lead (메인 세션)
  ...
  │   ├── research-lead (Explore/opus 계열 최신) — 조사 총괄
  │   ├── research-architect (Explore/opus 계열 최신) — 아키텍처 탐색
  │   └── research-analyst (Explore/opus 계열 최신) — 코드베이스 분석
  │
  ├── planner (Plan/opus 계열 최신) — 계획 수립
  ├── backend-dev (general-purpose/sonnet 계열 최신) — 백엔드 구현
  ├── frontend-dev (general-purpose/sonnet 계열 최신) — 프론트엔드 구현
  ├── verifier (Explore/opus 계열 최신) — 코드 리뷰
  └── tester (Bash/sonnet 계열 최신) — 테스트 실행
```

> **모델 승격 예외**: backend-dev / frontend-dev / tester가 큰 작업·리팩토링이 필요하다 판단하면 Team Leader에게 요청. Team Leader가 판단·승인 시 일시 opus 계열 승격 가능. 장애(API 529/overload) 시에는 요청 없이 Team Lead 판단으로 즉시 전환 가능(Phase 9-4 FB-01-1, 상세: SUB-SSOT TEAM-LEAD `2-lifecycle-procedure.md` §LIFECYCLE-5 RESPAWN Step 4).
...
[2] Task tool(team_name, name, subagent_type, model) × N  ← 팀원 스폰
  │   예: planner (Plan/opus 계열 최신), backend-dev (general-purpose/sonnet 계열 최신),
  │       frontend-dev (general-purpose/sonnet 계열 최신), verifier (Explore/opus 계열 최신),
  │       tester (Bash/sonnet 계열 최신)
  │
  ▼
...
## 8. SSOT 구조 — 프레임워크/프로젝트 분리

> 이식 가능성을 위해 프레임워크(core) 문서와 프로젝트별 문서를 분리한다.
> 현재 비파괴적 전환 상태: 기존 파일 위치를 유지하고 core/는 참조 README로 연결한다 (물리 이동 미수행).
> **분류의 정본은 [UPGRADE.md §1](UPGRADE.md)** — `install.sh --upgrade`가 이 분류대로 덮어쓰기/보존을 수행한다.

```
docs/SSOT/
├── core/                          ← 프레임워크 공통 레이어
│   ├── README.md                  (참조 허브)
│   ├── 6-rules-index.md           (규칙 통합 인덱스)
│   └── 7-shared-definitions.md    (공통 포맷 정의 — 6th)
├── 0-entrypoint.md ~ 5-automation.md, ROLES/, SUB-SSOT/,
│   QUALITY/, TEMPLATES/, refactoring/, tests/ …
│                                  ← 프레임워크 (업그레이드 시 덮어씀)
├── 1-project.md / 2-architecture.md / PERSONA/
│                                  ← 프로젝트별 (업그레이드 시 보존, 번들 원본은 예시)
└── VERSION.md
```
