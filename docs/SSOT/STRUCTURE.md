# ver6-2 (PoC, LIFECYCLE-6 도입) — 폴더·파일 구조 설명
...
> **목적**: ver6-2 (PoC, LIFECYCLE-6 도입) 번들 내 모든 폴더·파일의 **위치·역할·연결 관계·로딩 시점**을 한 화면에서 조회
...
| `UPGRADE.md` | (절차) | SSOT 업그레이드 가이드 — 프레임워크/프로젝트 파일 분리, `install.sh --upgrade` 절차 (2026-07-06 신설) | 이식 프로젝트 관리자 |
...
└── TEAM-LEAD/      (v1.2)          # Team Lead (메인 세션, AutoCycle 확장)
    ...
    └── 2-lifecycle-procedure.md      # §LIFECYCLE-5 RESPAWN + §LIFECYCLE-6 SCHEDULER 상세 (9-2 분리)
```
...
> **번들 미포함**: `_backup/`은 원본 저장소 전용 레거시 보존 폴더로, 포터블 번들(install.sh 설치본)에는 포함되지 않는다. 아래 트리는 이력 참조용.
...
**원칙**: 삭제하지 않고 이동(원본 저장소에서 보존 — 번들 미포함). SSOT 본체의 실행 참조는 SUB-SSOT 정본을 사용하고, `_backup/` 언급은 이력 표기로만 남긴다.
...
| **레거시 GUIDES 원본** | `docs/_backup/GUIDES/{role}-work-guide.md` (번들 미포함) |
...
| **AutoCycle TEMPLATES (5종)** | `docs/SSOT/TEMPLATES/development-plan-template.md` 외 4종 |
| **AutoCycle 체인 관리** | `docs/phases/phase-chain-autocycle.md` (런타임 생성 — 실행 예시는 원본 저장소 참조) |
| **ITERATION-BUDGET (토큰 상한)** | `docs/SSOT/core/7-shared-definitions.md §8` |
| **CHAIN-12 (Tech Debt 자동 로딩)** | `docs/SSOT/TEMPLATES/tech-debt-report.md §5` + `1-orchestration §Phase Chain` |
| **CHAIN-13 (직전 3 Phase 기억)** | `docs/SSOT/SUB-SSOT/TEAM-LEAD/1-orchestration-procedure.md §Phase Chain` |

---

**문서 관리**: v1.2, 2026-04-16. ver6-2 (PoC, LIFECYCLE-6 도입) v8.2 기준 (AutoCycle v1.1 · Phase-I 반영). 이후 구조 변경 시 본 문서 갱신 필수.
