...
> ⚠ **집행 방식 — 현재 수동 운영**: §2.3 미수신 처리·§3.2 에스컬레이션을 자동 실행하는 데몬/타이머는 **번들에 탑재되어 있지 않다** (5-automation.md §복구 참조). 타이머 기반 감지는 Team Lead가 팀원 응답 지연을 직접 인지하여 본 SLA 기준으로 판단·집행한다. 즉 팀원이 무응답(hang)이어도 자동으로 BLOCKED 전이되지 않으므로, Team Lead는 장시간 작업 시 주기적으로 팀원 상태를 점검해야 한다. (`5th_mode.event = true` 환경에서 자동화 도입 예정)
