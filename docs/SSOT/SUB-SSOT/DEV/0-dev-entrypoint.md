...
DEV SUB-SSOT의 원본 문서는 `_backup/GUIDES/DEV-work-guide/`에 레거시 참조용으로 보존된다 (2026-04-14 backup 이동 — **포터블 번들 미포함**, 원본 저장소 전용):
...
상세 참조가 필요한 경우에만 원본 파일을 추가 로딩한다 (번들 설치본에서는 원본이 없으므로 생략).
