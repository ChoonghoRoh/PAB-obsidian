# ver6-2 (PoC, LIFECYCLE-6 도입) SSOT 전체 가이드

**버전**: v8.2-renewal-6th | **릴리스**: 2026-04-13
...
│   ├── VERSION.md                            # 버전 이력 (v6.0→v8.2-renewal-6th)
│   │
│   │── ── CORE SSOT ── ──
...
│   │       └── 2-lifecycle-procedure.md (LIFECYCLE-5/6 상세)
│   │
│   │── ── ROLES & PERSONA ── ──
...
│   │
│   │── ── GUIDES (레거시 backup — SUB-SSOT 이관 완료, 번들 미포함) ── ──
│   ├── _backup/GUIDES/             # 정본은 SUB-SSOT/{ROLE}/ — 원본 백업은 원본 저장소 전용(번들 미포함)
...
| **1-project** | 프로젝트 정의 (번들 원본은 예시 — 루트 PROJECT.md 기반 재작성 권장, [UPGRADE.md §1.2](UPGRADE.md)). 팀 구성(Team Lead + 5 core + Research Team 3). 역할별 subagent_type/model 매핑. 코드 편집 도메인(`[BE]`/`[FE]`/`[DB]`/`[FS]`). Hub-and-Spoke 통신. Research Team 라이프사이클(5th) |
...
SUB-SSOT/DEV의 각 파일은 `_backup/GUIDES/DEV-work-guide/` 원본의 축약본 (원본은 번들 미포함 — 원본 저장소 전용):
...
| `1-orchestration-procedure` | 6단계 Phase 오케스트레이션, Gate 판정(G0~G4 소유·기준), 에이전트 라이프사이클(LIFECYCLE-1~6 **요약표만**), 지연 스폰, Phase Chain 운영, 외부 질의 대응(HR-1/EDIT-2) |
| `2-lifecycle-procedure` | §LIFECYCLE-5 RESPAWN(좀비 감지 시그널·respawn 상한 5회)·§LIFECYCLE-6 SCHEDULER(체크 스케줄러 arm/해제·BUILDING 진입 차단) **상세 절차** (Phase 9-2 분리) |
...
| 5 | _backup/GUIDES/DEV-work-guide/ (백업, 번들 미포함) — 필요 시 FROZEN 선언 유지 | 이중 경로 동기화 관리 |
...
| TEAM-LEAD | [TEAM-LEAD/0-lead-entrypoint.md](SUB-SSOT/TEAM-LEAD/0-lead-entrypoint.md) | [1-orchestration-procedure.md](SUB-SSOT/TEAM-LEAD/1-orchestration-procedure.md) · [2-lifecycle-procedure.md](SUB-SSOT/TEAM-LEAD/2-lifecycle-procedure.md) |

### 14.3 역할 & 가이드

| 역할 | PERSONA | ROLES | 레거시 GUIDE (번들 미포함) | SUB-SSOT (정본) |
|------|---------|-------|-------|----------|
| Team Lead | [LEADER.md](PERSONA/LEADER.md) | [team-lead.md](ROLES/team-lead.md) | — | TEAM-LEAD/ |
| Planner | [PLANNER.md](PERSONA/PLANNER.md) | [planner.md](ROLES/planner.md) | planner-work-guide.md | PLANNER/ |
| Backend Dev | [BACKEND.md](PERSONA/BACKEND.md) | [backend-dev.md](ROLES/backend-dev.md) | backend-work-guide.md | DEV/ |
| Frontend Dev | [FRONTEND.md](PERSONA/FRONTEND.md) | [frontend-dev.md](ROLES/frontend-dev.md) | frontend-work-guide.md | DEV/ |
| Verifier | [QA.md](PERSONA/QA.md) | [verifier.md](ROLES/verifier.md) | verifier-work-guide.md | VERIFIER/ |
| Tester | [QA.md](PERSONA/QA.md) | [tester.md](ROLES/tester.md) | tester-work-guide.md | TESTER/ |
| Research Lead | [RESEARCH_LEAD.md](PERSONA/RESEARCH_LEAD.md) | [research-lead.md](ROLES/research-lead.md) | research-work-guide.md | RESEARCH/ |
...
| DEV-work-guide 원본 (백업) | `_backup/GUIDES/DEV-work-guide/` (번들 미포함 — 원본 저장소 전용) |
...
**사용법**:
```bash
bash scripts/pmAuto/report_to_telegram.sh "MyProject" "Phase 1-2 완료: 사용자 인증 구현"
```

**인자**:
| 순번 | 인자 | 예시 |
|------|------|------|
| $1 | 프로젝트명 | `PAB-SSOT-Nexus` |
| $2 | 메시지 본문 (Markdown) | `Phase X-Y 완료: {요약}` |

**메시지 형식** (HR-8 규정):
```
[PAB-SSOT-Nexus] Phase {N}-{M} 완료: {1줄 요약}
결과: {핵심 수치}
보고서: {경로}
```
...
**문서 관리**: v1.0 | 2026-04-13 | ver6-2 (PoC, LIFECYCLE-6 도입) (v8.2-renewal-6th) 전체 가이드
