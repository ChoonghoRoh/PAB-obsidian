...
# /wiki — 원본 + 요약 두 파일 동시 생성 (Karpathy 3계층)
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🔧 /wiki  — Karpathy 3계층 wiki 노트 생성
   1회 호출 → 원본(SOURCE) + 요약본 두 파일 동시 생성

📥 사용법
   /wiki <내용 또는 URL...> [--type=TYPE] [--dry]
...
📚 예시
   /wiki https://gist.github.com/karpathy/... 내용 정리해줘
   /wiki 위에서 논의한 RAG 패턴 정리
   /wiki                   # 직전 대화 컨텍스트 활용
   /wiki <input> --type=LESSON --dry
...
- 예시: `/wiki https://gist.github.com/karpathy/... 정리해줘` → 요약=RESEARCH_NOTE, 원본=SOURCE.
...
### Step 8.5 — vault 초기 세팅 자동 부트스트랩 (신규 사용자 가드)

vault root 결정 직후, 저장 전에 **vault 구조를 검사**하고 누락분을 스킬 동봉 시드(`.claude/skills/wiki/vault-seed/`)로 자동 생성한다. 개인별 vault가 백지 상태여도 `/wiki` 한 번으로 문서규격에 맞는 저장이 보장된다.
...
3. **부트스트랩 발생 시 사용자 보고**:
   ```
   🧱 vault 초기 세팅 감지 — 폴더 N개 + 규격 파일 M개 생성 (<vault 경로>)
...
**복사 대상 (1개 디렉토리)**:
1. `.claude/skills/wiki/` 통째로 — `SKILL.md` + `PORTABILITY.md` + `vault-seed/` (규격 문서·템플릿·MOC 골격 동봉)

**이전 시 vault 구조 동기화**: 별도 수작업 불필요 — 첫 `/wiki` 호출 시 Step 8.5 부트스트랩이 누락 폴더·규격 파일을 시드에서 자동 생성한다 (기존 파일은 덮어쓰지 않음).
