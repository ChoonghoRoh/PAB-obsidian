...
3. Claude Code 세션 재시작 → `/wiki --help` 자동완성 확인
...
#    /wiki --help     # 자동완성 확인
#    /wiki <test>     # 첫 호출 → 본질 5항목 검증
```

**주의**: nexus는 `pab` namespace에 이미 다른 skill(`/phase-init`, `/report` 등)이 있을 수 있음. `/wiki`는 신규 skill 이름이라 충돌 없음.
...
첫 실제 호출:
```
/wiki https://example.com/some-article 정리해줘
```
...
| 증상 | 원인 | 해결 |
|---|---|---|
| `/wiki` 자동완성 안 됨 | namespace 미로드 | Claude Code 세션 재시작 |
| `Unknown skill: wiki` | plugin.json 위치/내용 오류 | `.claude-plugin/plugin.json` `name: "pab"` 확인 |
...
| `WebFetch` 차단 (Reddit 등) | 사이트별 인증 필요 | `curl + User-Agent` fallback (`/wiki` 본문에서 사용자가 수동 페치 후 입력) |
...
- ❌ 다중 SKILL 분리 (wiki-link-suggest, wiki-moc-update 등) — `/wiki` 1개로 통합
